...
for (let i = 0; i < 10; ++i) {
  ...
  describe("Hashnet server", () => {
    ...
    test("induceServerMigration", () => {
      const ns = getNS("hacknet-server-0");
      expect(() => ns.dnet.induceServerMigration("hacknet-server-0")).toThrow(
        "hacknet-server-0 is not a darknet server",
      );
    });
    test("isDarknetServer", () => {
      const ns = getNS("hacknet-server-0");
  ...
  describe("Non-existent server", () => {
    ...
    test("induceServerMigration", () => {
      const ns = getNsOnDarkWeb();
      expect(() => ns.dnet.induceServerMigration(hostnameOfNonExistentServer)).toThrow(
        errorMessageForNonExistentServer,
      );
    });
    test("isDarknetServer", () => {
      const ns = getNsOnDarkWeb();
  ...
  describe("Use IP instead of hostname", () => {
    ...
    test("getServerRequiredCharismaLevel", () => {
      const ns = getNsOnNonDarkwebDarknetServer();
      expect(ns.dnet.getServerRequiredCharismaLevel(ip)).toStrictEqual(
        getDarknetServerOrThrow(ip).requiredCharismaSkill,
      );
    });
  });
